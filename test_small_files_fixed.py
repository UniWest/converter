...
class TestSmallFileConversions(unittest.TestCase):
    ...
    def test_png_to_jpg_conversion(self):
        ...
        print(f"[OK] Конвертация успешна. Размер JPG: {output_path.stat().st_size} байт")
    ...
    def test_gif_optimization(self):
        ...
        print(f"[OK] Обработка GIF успешна. Размер: {output_path.stat().st_size} байт")
    ...
    def test_file_size_validation(self):
        ...
        print("[OK] Все размеры файлов прошли валидацию")
    ...
    def test_conversion_parameters(self):
        ...
        try:
            ...
            if result.success:
                from PIL import Image
                with Image.open(output_path) as img:
                    print(f"[OK] Конвертация с параметрами. Новый размер: {img.size}")
                    # Проверяем, что размер изменился
                    self.assertEqual(img.size, (20, 15))
            else:
                print(f"[INFO] Параметры не поддерживаются: {result.error_message}")
        except Exception as e:
            print(f"[INFO] Тест параметров пропущен: {e}")
    ...
    def test_error_handling_with_corrupted_files(self):
        ...
        print(f"[OK] Поврежденный файл правильно обработан. Ошибка: {result.error_message[:50]}...")
...
class TestAdapterPerformance(unittest.TestCase):
    ...
    def test_multiple_conversions_speed(self):
        ...
        print(f"[OK] Конвертировано {len(files)} файлов за {total_time:.3f} сек")
...
def main():
    ...
    try:
        ...
        print("[WARN] PIL/Pillow недоступен - некоторые тесты будут пропущены")
